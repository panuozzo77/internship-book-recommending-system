...
def signal_handler(sig, frame):
    ...
    if not shutdown_requested:
        print("\n--- Shutdown requested! ---")
        print("The script will finish the current BATCH and then exit gracefully.")
...
def execute_batch(db, book_ops, review_ops, progress_ops):
    ...
    print(f"  > Committing batch: {len(book_ops)} book ops, {len(review_ops)} review ops, {len(progress_ops)} progress ops...")
    start_time = time()
    ...
    end_time = time()
    print(f"  > Batch committed in {end_time - start_time:.2f} seconds.")
...
def deduplicate_books_fast():
    ...
    try:
        ...
        print(f"Found {len(all_duplicate_work_ids)} total duplicate work_ids.")
        print(f"Skipping {len(processed_work_ids)} already processed work_ids.")
        print(f"--> Starting processing for {total_to_process} remaining work_ids in batches of {BATCH_SIZE}.\n")
        ...
        for i, work_id in enumerate(work_ids_to_process):
            ...
            if len(progress_log_operations) >= BATCH_SIZE or shutdown_requested:
                print(f"Processing batch ending with work_id: {work_id} ({i+1}/{total_to_process})")
        ...
        print("\nForceful exit. Data for the last batch might not have been committed.", file=sys.stderr)
    finally:
        if shutdown_requested:
            print(f"\n--- Script stopped gracefully by user. ---")
            print(f"Processed a total of {total_processed_in_run} work_ids in this run.")
